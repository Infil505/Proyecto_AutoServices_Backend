...
describe('createJWT / verifyJWT', () => {
  it('round-trip: payload is recovered after verify', async () => {
  ...
  it('rejects a token with wrong signature', async () => {
    const token = await createJWT({ id: 1, exp: Math.floor(Date.now() / 1000) + 3600 }, SECRET);
    const tampered = token.slice(0, -5) + 'XXXXX';
    expect(await verifyJWT(tampered, SECRET)).toBeNull();
  });

  it('rejects an expired token', async () => {
    const token = await createJWT({ id: 1, exp: Math.floor(Date.now() / 1000) - 1 }, SECRET);
    expect(await verifyJWT(token, SECRET)).toBeNull();
  });

  it('rejects a token signed with a different secret', async () => {
    const token = await createJWT({ id: 1, exp: Math.floor(Date.now() / 1000) + 3600 }, 'other-secret');
    expect(await verifyJWT(token, SECRET)).toBeNull();
  });
